...
signed main() {
    // input
    double T, L, X, Y, Q; cin >> T >> L >> X >> Y >> Q;
    vector<double> ans(Q);
    rep (i, Q) {
        double e;
        cin >> e;
        double theta = double(e / T) * 2 * M_PI;
        double dz = -cos(theta) * double(L / 2) + double(L / 2);
        double dy = -sin(theta) * double(L / 2);
        double dxy = sqrt(powl(X, 2) + powl(Y - dy, 2));
        ans[i] = atan(dz / dxy) * (double(180) / M_PI);
    }
