...
signed main() {
    ...
    while (leaders.size() > 1) {
        auto [key, a, b] = E.back(); E.pop_back();
        int l_from = UF.leader(a);
        leaders.erase(l_from);
        int l_to = *leaders.begin();
        leaders.erase(l_to);
        UF.merge(l_from, l_to);
        int l = UF.leader(l_from);
        leaders.insert(l);
        ans.push_back({key, a+1, l_to+1});
    }
