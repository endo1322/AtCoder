...
map<char,pair<int, int>> MD4 = {{'L', {-1, 0}}, {'D', {0, -1}}, {'R', {1, 0}}, {'U', {0, 1}}};
...
signed main() {
    ...
    rep(i, N) dragon.push_back({i+1, 0});
    rep (i, Q) {
        int q; cin >> q;
        if (q == 1) {
            char C; cin >> C;
            pair<int, int> d = MD4[C];
            pair<int, int> p = dragon.front();
            dragon.push_front({p.first + d.first, p.second + d.second});
            dragon.pop_back();
        } else {
            int p; cin >> p;
