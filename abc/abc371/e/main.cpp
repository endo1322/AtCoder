...
signed main()
{
    ...
    cin >> N;
    V<V<int>> pos_x(N);
    rep(i, N) pos_x[i].push_back(0);
    rep(i, N)
    {
        int a;
        cin >> a;
        a--;
        pos_x[a].push_back(i + 1);
    }
    rep(i, N) pos_x[i].push_back(N + 1);
    ...
    rep(i, N)
    {
        rep3(j, 1, pos_x[i].size())
        {
            int idx_prev = pos_x[i][j - 1];
            int idx_next = pos_x[i][j];
            int dist = idx_next - idx_prev;
            XC2 += dist * (dist - 1) / 2;
        }
