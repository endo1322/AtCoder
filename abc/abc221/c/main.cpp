...
signed main() {
    ...
    int ans = MIN_INF;
    sort(all(N_str));
    rep3 (i, 1, (N_str.size() / 2) + 1) {
        do {
            string N_str_prev = N_str.substr(0, i);
            string N_str_next = N_str.substr(i);
            if (N_str_prev[0] == '0' || N_str_next[0] == '0') continue;
            int n_prev = stol(N_str_prev);
            int n_next = stol(N_str_next);
            ans = max(ans, n_prev * n_next);
        } while (next_permutation(all(N_str)));
    }
